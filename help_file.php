<?php
include('include/nav.php');
?>
<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Document</title>
    <link rel="stylesheet" href="css/styles/help_file.css">
</head>

<body>

    <div class="sidebar">
        <div class="active" id="file_sidebar">ملف</div>
        <div id="account_sidebar">حسابات</div>
        <div id="item_sidebar">مواد</div>
        <div id="category_sidebar">الأصناف</div>
        <div id="bills_sidebar">فواتير</div>
        <div id="bonds_sidebar">سندات</div>
        <div id="setting_sidebar">الإعدادات</div>
    </div>
    <div class="content" id="content">
        <section id="file_section">
            <div class="" id="">
                <h3 class="bd-title" id="content"> ملف </h3>
                <p class="bd-lead">
                    تتضمن هذه النافذة معلومات عن الملف المنشأ وتاريخ إنشائه وعن قاعدة البيانات ومعلومات عن
                    النسخ الاحتياطي والاستيراد
                    يمكن طلب هذه النافذة من القائمة--> ملف
                </p>
                <dl>
                    <dt> فتح ملف</dt>
                    <dd>
                        يمكننا إنشاء ملف جديد أو فتح ملف موجود سابقاً والتعامل معه
                    </dd>
                    <dt>استيراد نسخة احتياطية </dt>
                    <dd>
                        يمكننا استيراد نسخة احتياطية من خلال تحديد موقع النسخة التي قمنا بأخذها مسبقاً

                    </dd>
                    <dt>النسخ الاحتياطي </dt>
                    <dd>
                        من الضروري دوماً عمل نسخ احتياطية عن ملفاتك خوفاً من حدوث أخطاء أو من ضياع المعطيات نتيجة انقطاع
                        مفاجئ في التيار الكهربائي
                        أو من حدوث عطل ما نتيجة فيروس أو حذف الملفات من قبل أحد مستخدمي الحاسب عن طريق الخطأ
                        حيث يوفر البرنامج طريقة لتخزين النسخ الاحتياطية على سطح المكتب
                        <p>
                            نستطيع طلب (النسخ الاحتياطي) من القائمة --> ملف -->النسخ الاحتياطي
                        </p>
                    </dd>
                </dl>
                <p>
                <h6 class="c"> محتويات نافذة فتح ملف : </h6>
                <B class="c"> الحقول :</B>
                <table>
                    <tr>
                        <th style="width:17%"> اسم قاعدة البيانات </th>
                        <td style="height:30px">
                            يرجى إدخال اسم قاعدة البيانات التي تريد التعامل معها . مثال :اسم المؤسسة أو اسم الشركة ,أو اسم المحل
                        </td>
                    </tr>
                </table>

                <B class="c"> الأزرار :</B>

                <table>
                    <tr>
                        <th style="width:7%">
                            فتح
                        </th>
                        <td style="height:30px">
                            هذا الزر مخصص لفتح الملف المحدد سابقاً والتعامل معه
                        </td>
                    </tr>
                    <tr>
                        <th style="width:7%">
                            جديد
                        </th>
                        <td style="height:30px">
                            هذا الزر مخصص لإنشاء ملف جديد, يرجى إدخال اسم الملف المراد إنشاؤه في الخانه
                            الفارغة ثم الضغط على زر إضافة ليتم إضافته بشكل صحيح
                        </td>
                    </tr>
                    <tr>
                        <th style="width:7%">
                            حذف
                        </th>
                        <td style="height:30px">
                            هذا الزر مخصص لحذف الملف المحدد سابقاً
                        </td>
                    </tr>
                    <tr>
                        <th style="width:7%"  class="c">
                            ملاحظة
                        </th>
                        <td style="height:30px">
                             قاعدة البيانات الافتراضية لايمكن حذفها
                        </td>
                    </tr>
                    <tr>
                        <th style="width:7%">
                            إغلاق
                        </th>
                        <td style="height:30px">
                            يستخدم هذا الزر لغلق الملف الحالي
                        </td>
                    </tr>

                </table>
                </p>
            </div>
        </section>

        <section id="account_section">
            <div class="" id="">
                <h3 class="bd-title" id="content"> حسابات </h3>
                <p class="bd-lead">
                    يمكن طلب نافذة بطاقة الحساب لتعريف حسابات جديدة أو استعراض معلومات ل حسابات معرفة
                    سابقاً من القائمة --> حسابات
                </p>
                <dl>
                    <dt> بطاقة حساب </dt>
                    <dd>
                        من خلال بطاقة الحساب هذه تستطيع تعريف أو استعراض المعلومات الأساسية عن زبائنك أو مورديك
                        كالبيانات الشخصية ,وإضافة رصيد افتتاحي للعمل لهذه البطاقة
                    </dd>
                    <dt>كشف حساب </dt>
                    <dd>
                        يظهر هذا الكشف الحركات المالية التي أجريت على هذا الحساب  
                        ورصيد الحساب الحالي بشكل مختصر أو تفصيلي مع إمكانية طباعة الكشف بكل سهولة     
                    </dd>
                    <dt> قائمة حسابات </dt>
                    <dd>
                        يتم عرض معلومات جميع بطاقات الحسابات المعرفة
                         مسبقاً مع أرصدتها الحالية مبوبةً ضمن الحسابات الرئيسية لها 
                    </dd>
                    <dt> تقرير حركة كمسيون </dt>
                    <dd>
                        تعرض هذه الصفحة تقريراً مفصلاً عن حركة الكمسيون وفقاً لشروط محددة
                    </dd>
                </dl>

                <p>
                <h6 class="c"> محتويات نافذة بطاقة حساب: </h6>
                <b> معلومات الحساب </b>
                <B class="c"> حقولها :</B>
                <table>
                    <tr>
                        <th style="width:9%"> رمز الحساب </th>
                        <td style="height:30px">
                            هذه الخانة مخصصة لعرض رمز مميز للحساب
                        </td>
                    </tr>
                    <tr>
                        <th style="width:9%"> الحساب </th>
                        <td style="height:30px">
                            في هذه الخانة عليك إدخال اسم الزبون أو المورد الذي تقوم بتعريف بطاقة له
                        </td>
                    </tr>
                    <tr>
                        <th style="width:9%"> الحساب الرئيسي </th>
                        <td style="height:30px">
                                هنا يتم تحديد ما إذا كان الحساب المدخل حساب رئيسي سيتم إضافة حسابات فرعية عليه أم حساب فرعي تابع لحساب رئيسي معرف مسبقاً .مثال 
                                على حساب رئيسي (زبائن جبلة), مثال على حساب فرعي (المزارع عمار وهنا نختار الحساب الرئيسي زبائن جبلة فيصيح حساب المزارع عمار تابعاً للحساب الرئيسي زبائن جبلة)
                        </td>
                    </tr>
                </table>
                <b> الرصيد الافتتاحي </b>
                <B class="c"> حقولها :</B>
                <table>
                    <tr>
                        <th style="width:20%"> مدين </th>
                        <td style="height:30px">
                                هنا نضع قيمة المبلغ الذي هو حق لنا                
                        </td>
                    </tr>
                    <tr>
                        <th style="width:20%"> دائن </th>
                        <td style="height:30px">
                                    هنا نضع قيمة المبلغ الذي هو التزام علينا
                        </td>
                    </tr>
                </table>
                <b> معلومات التواصل </b>
                <B class="c"> حقولها :</B>

                <table>
                    <tr>
                        <th style="width:12%"> المحافظة </th>
                        <td style="height:30px">
                            في هذه الخانة عليك إدخال اسم المحافظة التي يسكن فيها الزبون أو المورد الذي تقوم بتعريف بطاقة
                            له

                        </td>
                    </tr>
                    <tr>
                        <th style="width:12%"> المدينة </th>
                        <td style="height:30px">
                            في هذه الخانة عليك إدخال اسم المدينة التي يسكن فيها الزبون أو المورد الذي تقوم بتعريف بطاقة
                            له

                        </td>
                    </tr>
                    <tr>
                        <th style="width:12%"> مكان السكن </th>
                        <td style="height:30px">
                            في هذه الخانة عليك إدخال مكان السكن بالتحديد للزبون أو المورد الذي تقوم بتعريف بطاقة له
                        </td>
                    </tr>
                    <tr>
                        <th style="width:12%"> الهاتف </th>
                        <td style="height:30px">
                            في هذه الخانة عليك إدخال رقم الهاتف للزبون أو المورد الذي تقوم بتعريف بطاقة له
                        </td>
                    </tr>
                    <tr>
                        <th style="width:12%"> ملاحظات </th>
                        <td style="height:30px">
                            يعدّ ملء هذا الحقل خياراً غير إجباري, حيث يتيح لك أن تدوّن أي نص تشعر بأنّه ضروري لهذا
                            الحساب

                        </td>

                    </tr>

                </table>

                <B class="c"> الأزرار :</B>

                <table>
                    <tr>
                        <th style="width:12%">
                            إضافة
                        </th>
                        <td style="height:30px">
                            يستخدم هذا الزر لتخزين المعلومات الحالية في النافذة كبطاقة الحساب (العميل) المحدد
                        </td>
                    </tr>
                    <tr>
                        <th style="width:12%">
                            تعديل
                        </th>
                        <td style="height:30px">
                            يستخدم هذا الزر لتعديل معلومات بطاقة الحساب (العميل) الحالي
                        </td>
                    </tr>
                    <tr>
                        <th style="width:12%">
                            حذف
                        </th>
                        <td style="height:30px">
                            يستخدم هذا الزر لحذف بطاقة الحساب (العميل) الحالي
                        </td>
                    </tr>
                    <tr>
                        <th style="width:12%" class ="c">
                            ملاحظة                    
                        </th>
                        <td style="height:30px">
                            لايمكن حذف بطاقة حساب ارتبطت بعمليات 
                        </td>
                    </tr>
                    <tr>
                        <th style="width:12%">
                            إغلاق
                        </th>
                        <td style="height:30px">
                            يستخدم هذا الزر لغلق بطاقة الحساب (العميل) الحالي
                        </td>
                    </tr>

                </table>
                </p>


                <P>
                <h6 class="c"> محتويات نافذة كشف حساب : </h6>
                <P>
                    تضمن هذه النافذة جدول بعدة أعمدة, عن تفاصيل الحسابات المدخلة من اسم الحساب المقابل
                    وقيمة المبلغ الذي يجب أن ندفعه لهذا الحساب وقيمة المبلغ الواجب دفعه لنا
                    مع تاريخ كل عملية ورصيد كل حركة

                </P>
                <B class="c"> الحقول :</B>
                <table>
                    <tr>
                        <th style="width:13%">
                            الحساب
                        </th>
                        <td style="height:30px">
                            للحصول على كشف تفصيلي لحساب معين ,حدد في هذه الخانة
                            اسم الحساب المراد عرض بياناته
                        </td>
                    </tr>
                    <tr>
                        <th style="width:13%">
                            العملة
                        </th>
                        <td style="height:30px">
                            هذا الزر مخصص ليتم اختيار العملة التي سوف تتم بها عمليات الشراء والمبيع
                        </td>
                    </tr>
                    <tr>
                        <th style="width:13%">
                            من تاريخ..إلى تاريخ
                        </th>
                        <td style="height:30px">
                            لدراسة ماتمّ تحريره من حسابات خلال فترة زمنية معينة
                            حدد في هاتين الخانتين تاريخ بداية ونهاية تلك الفترة
                        </td>
                    </tr>

                        <tr>
                            <th style="width:13%"> نوع التقرير </th>
                            <td style="height:30px">
                                يوفر لك البرنامج إمكانية الاختيار بين شكلين من أشكال نوافذ التقارير:
                            </td>

                            <tr>
                                <th style="width:9%" text-align=" center">     مختصر  </th>
                                <td style="height:30px" >
                                        يظهر لك هذا التقرير قيمة الفواتير بدون تفصيلها والعملاء فقط
                                </td>
                            </tr>
                            
                            <tr>
                                <th style="width:9%" text-align=" center"> تفصيلي </th>
                                <td style="height:30px">
                                    يظهر لك هذا التقرير تفصيل الفواتير بالكامل مع إمكانية التحكم بخيارات الإظهار
                                </td>
                            </tr>
                    </tr>

                    
                    <tr>
                        <th style="width:13%">
                            مجموع مدين
                        </th>
                        <td style="height:30px">
                            هو مجموع المبالغ المستحقة لنا 
                        </td>
                    </tr>
                    <tr>
                        <th style="width:13%">
                            مجموع دائن
                        </th>
                        <td style="height:30px">
                                هو مجموع المبالغ الالتزام علينا      
                        </td>
                    </tr>
                    <tr>
                        <th style="width:13%">
                                الرصيد                       
                        </th>
                        <td style="height:30px">
                        هو الرصيد الحالي لهذا الحساب والفرق بين ما لنا وما علينا في حال ظهر هذا الرصيد بدون إشارة (-) فهو حق لنا
                        وفي حال ظهر بجانبه إشارة (-) يكون التزام علينا 
                        </td>
                    </tr>
                </table>

                <B class="c"> الأزرار :</B>
                <table>
                    <tr>
                        <th style="width:12%">
                            معاينة
                        </th>
                        <td style="height:30px">
                            للحصول على دراسة تفصيلية لحساب معين ,حدد في هذه الخانة
                            اسم الحساب المراد البحث عنها
                        </td>
                    </tr>
                    <tr>
                        <th style="width:12%">
                            طباعة
                        </th>
                        <td style="height:30px">
                            هذا الزر مخصص لطباعة التقرير المحدد لحساب معين وفق الخيارات المحددة
                        </td>
                    </tr>
                    <tr>
                        <th style="width:12%">
                            إغلاق
                        </th>
                        <td style="height:30px">
                            هذا الزر مخصص لغلق نافذة كشف حساب
                        </td>
                    </tr>
                </table>
                </p>

                <P>
                <h6 class="c"> محتويات نافذة قائمة الحسابات : </h6>
                <P>
                        يتم عرض معلومات جميع بطاقات الحسابات المعرفة
                         مسبقاً مع أرصدتها الحالية مبوبةً ضمن الحسابات الرئيسية لها 
                </P>
                <B class="c"> الأزرار :</B>
                <table>
                    <tr>
                        <th style="width:12%">
                            بحث
                        </th>
                        <td style="height:30px">
                            للحصول على دراسة تفصيلية لحساب معين ,حدد في هذه الخانة
                            اسم الحساب المراد البحث عنها
                        </td>
                    </tr>
                    <tr>
                        <th style="width:12%">
                            حساب جديد
                        </th>
                        <td style="height:30px">
                            هذا الزر مخصص ليتم نقلنا لواجهة بطاقة حساب لإضافة معلومات حساب جديد
                        </td>
                    </tr>
                    <tr>
                        <th style="width:12%">
                            كشف                        
                        </th>
                        <td style="height:30px">
                            هذا الزر مخصص ليأخذنا لكشف حساب تفصيلي للحساب المحدد   
                        </td>
                    </tr>
                    <tr>
                        <th style="width:12%">
                            إغلاق
                        </th>
                        <td style="height:30px">
                            هذا الزر مخصص لغلق نافذة قائمة الحسابات
                        </td>
                    </tr>
                </table>
                </p>

                <p>
                <h6 class="c"> محتويات نافذة تقرير حركة كمسيون : </h6>
                <P>
                    تضمن هذه النافذة جدول بعدة أعمدة, عن تفاصيل الفاتورة كاملة من تاريخ إصدار الفاتورة
                    والمواد المباعة مع أسماء العملاء وقيمة الكمسيون الخاصة بهذه العملية
                </P>
                <B class="c"> الحقول :</B>
                <table>
                    <tr>
                        <th style="width:18%">
                            إجمالي الفواتير
                        </th>
                        <td style="height:30px">
                            عرض القيمة المالية الإجمالية للفواتير
                        </td>
                    </tr>
                    <tr>
                        <th style="width:18%">
                            قيمة الكمسيون
                        </th>
                        <td style="height:30px">
                            يتم تحديد قيمة الكمسيون المراد حسمها من القيمة الإجمالية للفواتير
                        </td>
                    </tr>
                    <tr>
                        <th style="width:18%">
                            صافي الفواتير
                        </th>
                        <td style="height:30px">
                            عرض القيمة المالية للفواتير بعد إنقاص قيمة الكمسيون من القيمة الإجمالية للفواتير

                        </td>
                    </tr>
                </table>
                <B class="c"> الأزرار :</B>
                <table>
                    <tr>
                        <th style="width:5%">
                            بحث
                        </th>
                        <td style="height:30px">

                            للحصول على دراسة تفصيلية لحركة الكمسيون وفقاً لإحدى المواد أو الأصناف أو العملاء ,حدد في هذه
                            الخانة اسم المادة أو الصنف أو العميل ,حيث سيتم دراسة ماتم تحريره من فواتير خلال فترة زمنية
                            معينه وفقاً لما تم تحديده في في خانة تاريخ البداية والنهاية

                        </td>
                    </tr>
                    <tr>
                        <th style="width:12%">
                            إغلاق
                        </th>
                        <td style="height:30px">
                            هذا الزر مخصص لغلق نافذة تقرير حركة كمسيون 
                        </td>
                    </tr>
                    <tr>
                        <th style="width:5%">
                            طباعة
                        </th>
                        <td style="height:30px">
                            هذا الزر مخصص لطباعة تقرير مفصل لحركة الكمسيون وفق الخيار المحدد
                        </td>
                    </tr>

                </table>
                </p>
            </div>
        </section>

        <section id="item_section">
            <div class="" id="">
                <h3 class="bd-title" id="content"> مواد </h3>
                <p class="bd-lead">
                    يمكن طلب نافذة مواد لتعريف مواد و أصناف جديدة
                </p>

                <dl>
                    <dt>بطاقة صنف</dt>
                    <dd>
                        يجب عليك إدخال بطاقة لكل صنف تتعامل معه في عمليات البيع والشراء ويتم إدخال بطاقات الأصناف عن
                        طريق نافذة بطاقة صنف التي يمكن طلبها من القائمة --> مواد --> بطاقة صنف
                    </dd>
                    <dt>بطاقة مادة</dt>
                    <dd>
                        يجب عليك إدخال بطاقة لكل مادة تتعامل معها في عمليات البيع والشراء وتحديد هذه البطاقة لأي بطاقة صنف تتبع , ويتم إدخال بطاقات المواد عن
                        طريق نافذة بطاقة مادة التي يمكن طلبها من القائمة --> مواد --> بطاقة مادة
                    </dd>
                    <dt>قائمة المواد </dt>
                    <dd>
                        لائحة لعرض كافة المواد
                    </dd>
                    <dt>تقرير حركة مادة </dt>
                    <dd>
                        تعرض هذه الصفحة تقريراً موجزاً عن حركة المادة المحددة
                    </dd>
                </dl>



                <p id=" ">
                <h6 class="c"> محتويات نافذة بطاقة صنف: </h6>
                <B class="c"> الحقول :</B>

                <table>
                    <tr>
                        <th style="width:18%"> رمز الصنف </th>
                        <td style="height:30px">
                            هذه الخانة مخصصة لعرض رمز مميز للصنف </td>
                    </tr>
                    <tr>
                        <th style="width:20%"> اسم الصنف </th>
                        <td style="height:30px">
                            هذه الخانة مخصصة لإدخال اسم الصنف
                        </td>
                    </tr>
                    <tr>
                        <th style="width:18%"> ملاحظات </th>
                        <td style="height:30px">
                            دون في هذه الخانة ملاحظات حول الصنف الذي تم إدخاله
                        </td>
                    </tr>
                </table>

                <B class="c"> الأزرار :</B>

                <table>
                    <tr>
                        <th style="width:18%">
                            إضافة
                        </th>
                        <td style="height:30px">
                            إضافة بيانات الصنف المدخل لقائمة الأصناف
                        </td>
                    </tr>
                    <tr>
                        <th style="width:18%">
                            تعديل
                        </th>
                        <td style="height:30px">
                            تعديل بيانات الصنف المدخل
                        </td>
                    </tr>
                    <tr>
                        <th style="width:18%">
                            بطاقة مادة                 
                        </th>
                        <td style="height:30px">
                        هذا الزر مخصص لينقلنا لواجهة بطاقة مادة  لإدخال مواد جديدة   
                        </td>
                    </tr>
                    <tr>
                        <th style="width:18%">
                            حذف
                        </th>
                        <td style="height:30px">
                            حذف الصنف المدخل من قائمة الأصناف
                        </td>
                    </tr>
                    <tr>
                        <th style="width:18%">
                            إغلاق
                        </th>
                        <td style="height:30px">
                            إغلاق نافذة بطاقة صنف
                        </td>
                    </tr>

                </table>
                </p>
                <p>
                <h6 class="c"> محتويات نافذة بطاقة المادة : </h6>
                <B class="c"> الحقول :</B>
                <table>
                    <tr>
                        <th style="width:18%"> رمز المادة </th>
                        <td style="height:30px">
                            هذه الخانة مخصصة لعرض رمز مميز
                        </td>
                    </tr>
                    <tr>
                        <th style="width:18%"> اسم المادة </th>
                        <td style="height:30px">
                            هذه الخانة مخصصة لإدخال اسم المادة
                        </td>
                    </tr>
                    <tr>
                        <th style="width:18%"> وحدة القياس </th>
                        <td style="height:30px">
                            هذه الخانة مخصصة لإدخال وحدة قياس المادة (غرام- سنتيمتر- قطعة-عبوة...)
                        </td>
                    </tr>
                    <tr>
                        <th style="width:18%"> الصنف </th>
                        <td style="height:30px">
                            هذه الخانة مخصصة لإدخال الصنف الذي تتبع له المادة
                        </td>
                    </tr>
                    <tr>
                        <th style="width:18%"> ملاحظات </th>
                        <td style="height:30px">
                            دوّن في هذه الخانة ملاحظات حول المادة التي تمّ إدخاله
                        </td>
                    </tr>
                </table>

                <B class="c"> الأزرار :</B>

                <table>
                    <tr>
                        <th style="width:22%">
                            استعراض المواد
                        </th>
                        <td style="height:30px">
                            ينقلنا لواجهة قائمة المواد لعرض جميع المواد المدخلة
                        </td>
                    </tr>
                    <tr>
                        <th style="width:22%">
                            إضافة صنف
                        </th>
                        <td style="height:30px">
                            ينقلنا لواجهة بطاقة صنف لإدخال صنف جديد لقائمة الأصناف
                        </td>
                    </tr>
                    <tr>
                        <th style="width:22%">
                            إضافة
                        </th>
                        <td style="height:30px">
                            إضافة بيانات المادة المدخلة لقائمة المواد
                        </td>
                    </tr>
                    <tr>
                        <th style="width:22%">
                            تعديل
                        </th>
                        <td style="height:30px">
                            تعديل بيانات المادة المدخلة
                        </td>
                    </tr>
                    <tr>
                        <th style="width:22%">
                            حذف
                        </th>
                        <td style="height:30px">
                            حذف المادة المدخلة من قائمة المواد
                        </td>
                    </tr>
                    <tr>
                        <th style="width:22%">
                            إغلاق
                        </th>
                        <td style="height:30px">
                            إغلاق نافذة بطاقة مادة
                        </td>
                    </tr>
                </table>
                </p>

                <P>
                <h6 class="c"> محتويات نافذة قائمة المواد : </h6>
                <P>
                    تضمن هذه النافذة جدول بعدة أعمدة, عن تفاصيل المواد المدخلة من رمز المادة
                    واسم المادة المدخلة باإضافة للصنف الذي تتبع له المادة مع العمليات الموافقة لهذه المادة
                </P>
                <B class="c"> الأزرار :</B>
                <table>
                    <tr>
                        <th style="width:11%">
                            بحث </th>
                        <td style="height:30px">
                            للحصول على دراسة تفصيلية لمادة معينة ,حدد في هذه الخلنة
                            اسم المادة المراد البحث عنها
                        </td>
                    </tr>
                    <tr>
                        <th style="width:12%">
                            صنف جديد
                        </th>
                        <td style="height:30px">
                            هذا الزر مخصص ليتم نقلنا لواجهة بطاقة صنف ليتم إضافة صنف جديد
                        </td>
                    </tr>
                    <tr>
                        <th style="width:11%">
                            بطاقة مادة
                        </th>
                        <td style="height:30px">
                            هذا الزر مخصص ليتم نقلنا لواجهة بطاقة مادة ليتم إضافة مادة جديدة
                        </td>
                    </tr>
                    <tr>
                        <th style="width:11%">
                                إغلاق 
                        </th>
                        <td style="height:30px">
                            هذا الزر مخصص لغلق نافذة قائمة المواد        
                        </td>
                    </tr>
                    <tr>
                        <th style="width:11%">
                                تعديل 
                        </th>
                        <td style="height:30px">
                            هذا الزر مخصص لفتح بطاقة مادة لتعديل بيانات المادة المحددة           
                        </td>
                    </tr>
                    <tr>
                        <th style="width:11%">
                                حذف 
                        </th>
                        <td style="height:30px">
                            هذا الزر مخصص لحذف  المادة المحددة               
                        </td>
                    </tr>
                </table>

                </p>
                <p>
                <h6 class="c"> محتويات نافذة تقرير حركة مادة : </h6>
                <P>
                    تضمن هذه النافذة جدول بعدة أعمدة, التاريخ الذي صدرت فيه الفاتورة
                    رقم الفاتورة ويندرج تحته مجمل العمليات الحاصلة على المادةالمحددة من بيع وشراء وإدخالات وإخراجات مع
                    الكمية والسعر الموافقة لكل عملية
                </P>


                <B class="c"> الأزرار :</B>
                <table>
                    <tr>
                        <th style="width:5%">
                            بحث
                        </th>
                        <td style="height:30px">
                            للحصول على دراسة تفصيلية لحركة إحدى المواد أو الأصناف أو العملاء ,حدد في هذه
                            الخانة اسم المادة أو الصنف أو العميل ,حيث سيتم دراسة ماتم تحريره من فواتير خلال فترة زمنية
                            معينه وفقاً لما تم تحديده في في خانة تاريخ البداية والنهاية
                        </td>
                    </tr>
                    <tr>
                        <th style="width:5%">
                            طباعة
                        </th>
                        <td style="height:30px">
                            طباعة تقرير مفصل لحركة وفق المادة المحددة
                        </td>
                    </tr>
                    <tr>
                        <th style="width:5%">
                            إغلاق
                        </th>
                        <td style="height:30px">
                        هذا الزر مخصص لغلق نافذة تقرير حركة مادة          
                        </td>
                    </tr>

                </table>
                </p>
            </div>
        </section>

        <section id="category_section">
            <p id="">
            <dl>
                <dt>بطاقة صنف</dt>
                <dd>
                    يجب عليك إدخال بطاقة لكل صنف تتعامل معه في عمليات البيع والشراء ويتم إدخال بطاقات الأصناف عن طريق
                    نافذة بطاقة صنف التي يمكن طلبها من القائمة --> مواد--> بطاقة صنف
                </dd>
            </dl>

            <h6 class="c"> محتويات نافذة بطاقة صنف: </h6>
            <B class="c"> الحقول :</B>

            <table>
                <tr>
                    <th style="width:18%"> رمز الصنف </th>
                    <td style="height:30px">
                        هذه الخانة مخصصة لعرض رمز مميز للصنف </td>
                </tr>
                <tr>
                    <th style="width:20%"> اسم الصنف </th>
                    <td style="height:30px">
                        هذه الخانة مخصصة لإدخال اسم الصنف
                    </td>
                </tr>
                <tr>
                    <th style="width:18%"> ملاحظات </th>
                    <td style="height:30px">
                        دون في هذه الخانة ملاحظات حول الصنف الذي تم إدخاله
                    </td>
                </tr>
            </table>

            <B class="c"> الأزرار :</B>

            <table>
                <tr>
                    <th style="width:18%">
                        إضافة
                    </th>
                    <td style="height:30px">
                        إضافة بيانات الصنف المدخل لقائمة الأصناف
                    </td>
                </tr>
                <tr>
                    <th style="width:18%">
                        تعديل
                    </th>
                    <td style="height:30px">
                        تعديل بيانات الصنف المدخل
                    </td>
                </tr>
                <tr>
                    <th style="width:18%">
                        حذف
                    </th>
                    <td style="height:30px">
                        حذف الصنف المدخل من قائمة الأصناف
                    </td>
                </tr>
                <tr>
                    <th style="width:18%">
                        إغلاق
                    </th>
                    <td style="height:30px">
                        إغلاق نافذة بطاقة صنف
                    </td>
                </tr>

            </table>
            </p>
        </section>

        <section id="bills_section">
            <div class="" id="">
                <h3 class="bd-title" id="content"> فواتير </h3>
                <p class="bd-lead">
                    البرنامج مخصص للتعامل مع نوع واحد من الفواتير وهي فاتورة كمسيون التي هي مخصصة 
                        لبيع مباشر لعملية شراء مباشرة
                </p>
                <dl>
                    <dt> فاتورة جديدة </dt>
                    <dd>
                            هنا نقوم بإنشاء فاتورة جديدة ضمن خيارات تحدد لهذه الفاتورة مثل البائع والمشتري والمواد...الخ
                    </dd>
                    <dt> قائمة الفواتير </dt>
                    <dd>
                        يتم عرض معلومات جميع الفواتير الموجودة
                    </dd>
                </dl>

                <p>
                <h6 class="c"> محتويات نافذة فاتورة جديدة: </h6>
                <table>
                    <tr>
                        <th style="width:8%"> رقم الفاتورة </th>
                        <td style="height:30px">

                            ستظهر ضمن هذه الخانة رقم الفاتورة الحالية ,وستتمكن من الانتقال إلى الفواتير التي تلي أو تسبق
                            هذه الفاتورة باستخدام أزرار الانتقال أو بتدوين رقم الفاتورة المطلوبة ضمن الخانة </td>
                        </td>
                    </tr>
                    <tr>
                        <th style="width:8%"> تاريخ الفاتورة </th>
                        <td style="height:30px">
                            تستطيع إدخال تاريخ تحرير الفاتورة في هذه الخانة ,و يظهر فيها بشكل تلقائي التاريخ الحالي,
                            وتستطيع تغييرالتاريخ باستخدام نافذة التقويم التي ستظهر بعد النقر على الزر المجاور للخانة
                        </td>
                    </tr>
                </table>


                <b> معلومات البائع </b>
                <table>
                    <tr>
                        <th style="width: 8%;"> البائع </th>
                        <td style="height:30px">
                           يتم إدخال اسم البائع الذي تمت عن طريقه عملية الشراء
                            المقصود بالبائع هنا هو من قام ببيعنا) 
                            (البضاعة أي نحن قمنا بشراء البضائع منه ,مثال المزارع الذي نشتري محصوله

                        </td>
                    </tr>
                    <tr>
                        <th style="width: 8%;"> طريقة الدفع </th>
                        <td style="height:30px">
                            يتيح لنا البرنامج حرية اختيار طريقة الدفع ( نقدي أو آجل )
                        </td>
                    </tr>
                    <tr>
                        <th style="width: 8%;"> ملاحظات </th>
                        <td style="height:30px">
                            دون في هذه الخانة ملاحظات حول عملية الشراء
                        </td>
                    </tr>
                </table>

                <b> معلومات المشتري </b>
                <table>
                    <tr>
                        <th style="width: 8%;"> المشتري </th>
                        <td style="height:30px">
                           يتم إدخال اسم المشتري الذي تمت عن طريقه عملية البيع
                        (المقصود بالمشتري هنا هو من قمنا بتحويل ماقمنا بشرائه من المزارع له من خلال عملية بيع مباشر)
                        </td>
                    </tr>
                    <tr>
                        <th style="width: 8%;"> طريقة الدفع </th>
                        <td style="height:30px">
                            يتيح لنا البرنامج حرية اختيار طريقة الدفع ( نقدي أو آجل )
                        </td>
                    </tr>
                    <tr>
                        <th style="width: 8%;"> ملاحظات </th>
                        <td style="height:30px">
                            دون في هذه الخانة ملاحظات حول عملية البيع
                        </td>
                    </tr>
                </table>



                <dl>
                    <dt> معلومات جدول الفاتورة</dt>
                    <dd>

                        جدول يحوي معلومات الفاتورة كرقم الفاتورة و اسم المادة مع معلومات عن الوزن القائم و الصافي و
                        السعر الإفرادي و الإجمالي
                    </dd>
                </dl>




                <B class="c"> الحقول :</B>
                <table>
                    <tr>
                        <th style="width:17%"> الإجمالي </th>
                        <td style="height:30px">
                            يتم عرض القيمة الإجمالية الكاملة للفاتورة </td>
                    </tr>
                    <tr>
                        <th style="width:17%"> الكمسيون </th>
                        <td style="height:30px">
                            يتم تحديد قيمة الكمسيون للفاتورة </td>
                    </tr>
                    <tr>
                        <th style="width:17%"> الصافي </th>
                        <td style="height:30px">
                            يتم عرض القيمة الصافية الكاملة للفاتورة بعد حسم الكمسيون </td>
                    </tr>
                </table>

                <B class="c"> الأزرار :</B>

                <table>
                    <tr>
                        <th style="width:10%">
                            (+)
                        </th>
                        <td style="height:30px">
                            هذا الزر مخصص لإضافة أسطر للفاتورة الحالية
                        </td>
                    </tr>
                    <tr>
                        <th style="width:10%">
                            (%)
                        </th>
                        <td style="height:30px">
                            هذا الزر يعطي للمستخدم حرية تحديد قيمة الخصم من الوزن القائم ليعطي اتوماتيكياً الوزن الصافي
                        </td>
                    </tr>

                    <tr>
                        <th style="width:10%">
                            حفظ
                        </th>
                        <td style="height:30px">
                            هذا الزر مخصص لحفظ الفاتورة المدخلة
                        </td>
                    </tr>
                    <tr>
                        <th style="width:10%">
                            طباعة بائع
                        </th>
                        <td style="height:30px">
                            يستخدم هذا الزر لطباعة الفاتورة الحالية الخاصة بالبائع(المزارع)
                        </td>
                    </tr>
                    <tr>
                        <th style="width:10%">
                            طباعة مشتري
                        </th>
                        <td style="height:30px">
                            يستخدم هذا الزر لطباعة الفاتورة الحالية الخاصة بالمشتري
                        </td>
                    </tr>

                </table>
                </p>
                <P>
                <h6 class="c"> محتويات نافذة قائمة الفواتير : </h6>
                <P>
                    تضمن هذه النافذة جدول بعدة أعمدة, عن تفاصيل الفواتير المدخلة من رقم الفاتورة واسم البائع والمشتري
                    وصافي الربح والإجمالي من كل فاتورة مع حالة الفاتورة مباعة أو غير مباعة

                </P>
                <B class="c"> الأزرار :</B>
                <table>
                    <tr>
                        <th style="width:5%">
                            فلترة
                        </th>
                        <td style="height:30px">
                            للحصول على بيانات جميع الفواتير نقوم بتحديد جميع الفواتير سواء مباعة أو غير مباعة
                        </td>
                        <th style="width:5%">
                        ,
                        </th>
                        <td style="height:30px">
                            للحصول على بيانات الفواتير المباعة فقط نقوم بتحديد الفواتير المباعة
                        </td>
                        <th style="width:5%">
                        ,
                        </th>
                        <td style="height:30px">
                            للحصول على بيانات الفواتير الغير مباعة فقط نقوم بتحديد الفواتير الغير المباعة
                        </td>
                    </tr>
                </table>
                </p>
            </div>
        </section>

        <section id="bonds_section">
            <div class="" id="">
                <h3 class="bd-title" id="content"> سندات </h3>
                <p class="bd-lead">
                    تستخدم السندات المتوفرة ضمن البرنامج لتدوين العمليات اليومية المختلفة
                    من قبض أو دفع وما إلى ذلك
                </p>

                <dl>
                    <dt>سند قبض</dt>
                    <dd>
                        يمكنك إدخال الدفعات التي يدفعها لك الزبائن باستخدام سند القبض, الذي تستطيع الوصول
                        إليه من القائمة --> سندات --> سند قبض
                    </dd>
                    <dt>سند دفع</dt>
                    <dd>
                        يمكنك إدخال الدفعات التي يتم تسديدها للموردين  باستخدام سند الدفع, الذي تستطيع الوصول
                        إليه من القائمة --> سندات --> سند دفع
                    </dd>
                </dl>

                <p>
                <h6 class="c"> محتويات نافذة سند القبض: </h6>
                <B class="c"> الحقول :</B>

                <table>
                    <tr>
                        <th style="width:10%"> رقم الإيصال </th>
                        <td style="height:30px">
                            يتم توليد رقم خاص لكل إيصال
                        </td>
                    </tr>
                    <tr>
                        <th style="width:10%"> الحساب </th>
                        <td style="height:30px">
                            هنا تستطيع وضع الحساب الذي سوف يتم إدخال المبلغ فيه (الحساب المدين ), يظهر
                            حساب الصندوق بشكل افتراضي ,مع ملاحظة أنك تستطيع تغيير الحساب الافتراضي
                        </td>
                    </tr>
                    <tr>
                        <th style="width:10%"> التاريخ </th>
                        <td style="height:30px">
                            هنا يوضع التاريخ الذي تم فيه استلام هذا المبلغ
                        </td>
                    </tr>
                    <tr>
                        <th style="width:10%"> العملة </th>
                        <td style="height:30px">
                            حدّد في هذه الخانة العملة الخاصة بالحساب الرئيسي والذي سيتحرك وفقها هذا الحساب
                        </td>
                    </tr>
                    <tr>
                        <th style="width:10%"> ملاحظات </th>
                        <td style="height:30px">
                            دوّن في هذه الخانة ملاحظات حول سند القبض الحالي
                        </td>
                    </tr>
                    <tr>
                        <th style="width:10%"> المجموع </th>
                        <td style="height:30px">
                            عرض مجموع المبالغ الدائنة الواجب دفعها لهذا الحساب
                        </td>
                    </tr>
                    <tr>
                        <th style="width:10%"> جدول الحسابات </th>
                        <td style="height:30px">
                            سيظهر في الجدول الأعمدة التي اختيرت في خصائص هذا النمط من السندات وفيه الحقول التالية:

                        </td>
                        <Table>
                            <th style="width:4%"> الرقم </th>
                            <td style="height:30px">
                                رقم ترتيبي
                            </td>

                            <th style="width:6%"> الدائن </th>
                            <td style="height:30px">
                                ضع فيه المبلغ الذي تم استلامه من العميل الذي سوف تضع اسم حسابه في حقل الحساب التالي لهذا
                                الحقل
                            </td>
                            <th style="width:6%"> الحساب </th>
                            <td style="height:30px">
                                ضع في هذا الحقل اسم الحساب الذي تم استلام المبلغ منه (الحساب الدائن)
                            </td>
                            <th style="width:6%"> ملاحظات </th>
                            <td style="height:30px">
                                يعدّ ملء هذا الحقل خياراً غير إجباري, حيث يتيح لك أن تدوّن أي نص تشعر بأنّه ضروري لشرح
                                هذه العملية
                            </td>

                        </Table>
                    </tr>

                </table>

                <B class="c"> الأزرار :</B>

                <table>
                    <tr>
                        <th style="width:15%">
                            إضافة
                        </th>
                        <td style="height:30px">
                            هذا الزر مخصص لإضافة بيانات سند القبض الحالي
                        </td>
                    </tr>
                    <tr>
                        <th style="width:15%">
                            طباعة
                        </th>
                        <td style="height:30px">
                            تستطيع طباعة سند القبض الحالي بالنقر على هذا الزر
                        </td>
                    </tr>
                    <tr>
                        <th style="width:15%">
                            تعديل
                        </th>
                        <td style="height:30px">
                            هذا الزر مخصص لتعديل بيانات سند القبض
                        </td>
                    </tr>
                    <tr>
                        <th style="width:15%">
                            حذف
                        </th>
                        <td style="height:30px">
                            هذا الزر مخصص لحذف سند القبض الحالي
                        </td>
                    </tr>
                    <tr>
                        <th style="width:15%">
                            جديد
                        </th>
                        <td style="height:30px">
                            هذا الزر مخصص ليولد سند قبض جديد
                        </td>
                    </tr>
                    <tr>
                        <th style="width:15%">
                            إغلاق
                        </th>
                        <td style="height:30px">
                            هذا الزر مخصص لغلق سند القبض الحالي
                        </td>
                    </tr>

                </table>
                </p>
                <p>
                <h6 class="c"> محتويات نافذة سند الدفع: </h6>
                <B class="c"> الحقول :</B>

                <table>
                    <tr>
                        <th style="width:10%"> رقم الإيصال </th>
                        <td style="height:30px">
                            يتم توليد رقم خاص لكل إيصال
                        </td>
                    </tr>
                    <tr>
                        <th style="width:10%"> الحساب </th>
                        <td style="height:30px">
                            تستطيع في هذه الخانة إدخال اسم الحساب الذي تمّ سحب المبلغ منه(الحساب الدائن)
                        </td>
                    </tr>
                    <tr>
                        <th style="width:10%"> التاريخ </th>
                        <td style="height:30px">
                            حدد في هذه الخانة التاريخ الذي تمّ فيه تسليم هذا المبلغ
                        </td>
                    </tr>
                    <tr>
                        <th style="width:10%"> العملة </th>
                        <td style="height:30px">
                            حدّد في هذه الخانة العملة الخاصة بالحساب الرئيسي والذي سيتحرك وفقها هذا الحساب
                        </td>
                    </tr>
                    <tr>
                        <th style="width:10%"> ملاحظات </th>
                        <td style="height:30px">
                            دوّن في هذه الخانة ملاحظات حول إيصال الدفع الحالي
                        </td>
                    </tr>
                    <tr>
                        <th style="width:10%"> المجموع </th>
                        <td style="height:30px">
                            عرض مجموع المبالغ المدينة الواجب دفعه لنا من قِبَل هذا الحساب
                        </td>
                    </tr>
                    <tr>
                        <th style="width:10%"> جدول الحسابات </th>
                        <td style="height:30px">
                            سيظهر في الجدول الأعمدة التي اختيرت في خصائص هذا النمط من السندات وفيه الحقول التالية:

                        </td>
                        <Table>
                            <th style="width:4%"> الرقم </th>
                            <td style="height:30px">
                                رقم ترتيبي
                            </td>

                            <th style="width:6%"> المدين </th>
                            <td style="height:30px">
                                ضع في هذا الحقل المبلغ الذي تمّ تسليمه للحساب الذي وضعته في حقل الحساب
                            </td>
                            <th style="width:6%"> الحساب </th>
                            <td style="height:30px">
                                ضع في هذا الحقل اسم الحساب الذي تمّ تسليمه المبلغ (الحساب المدين)
                            </td>
                            <th style="width:6%"> ملاحظات </th>
                            <td style="height:30px">
                                يعدّ ملء هذا الحقل خياراً غير إجباري, حيث يتيح لك أن تدوّن أي نص تشعر بأنّه ضروري لشرح
                                هذه العملية
                            </td>

                        </Table>
                    </tr>


                </table>

                <B class="c"> الأزرار :</B>
                <table>
                    <tr>
                        <th style="width:15%">
                            إضافة
                        </th>
                        <td style="height:30px">
                            هذا الزر مخصص لإضافة بيانات سند الدفع الحالي
                        </td>
                    </tr>
                    <tr>
                        <th style="width:15%">
                            طباعة
                        </th>
                        <td style="height:30px">
                            تستطيع طباعة سند الدفع الحالي بالنقر على هذا الزر
                        </td>
                    </tr>
                    <tr>
                        <th style="width:15%">
                            تعديل
                        </th>
                        <td style="height:30px">
                            هذا الزر مخصص لتعديل بيانات سند الدفع
                        </td>
                    </tr>
                    <tr>
                        <th style="width:15%">
                            حذف
                        </th>
                        <td style="height:30px">
                            هذا الزر مخصص لحذف سند الدفع الحالي
                        </td>
                    </tr>
                    <tr>
                        <th style="width:15%">
                            جديد
                        </th>
                        <td style="height:30px">
                            هذا الزر مخصص ليولد سند دفع جديد
                        </td>
                    </tr>
                    <tr>
                        <th style="width:15%">
                            إغلاق
                        </th>
                        <td style="height:30px">
                            هذا الزر مخصص لغلق سند الدفع الحالي
                        </td>
                    </tr>
                </table>
                </p>
            </div>
        </section>

        <section id="setting_section" style="margin-bottom: 100px;">
            <div class="" id="">
                <h3 class="bd-title" id="content"> الإعدادات </h3>
                <p>
                    بعد تخزين الفاتورة أو عند نقر زر طباعة تظهر نافذة طباعة فاتورة أو سند والتي يوفر لك تطبيقنا
                    العديد من الخيارات التي تتيح لك تحديد الشكل أو التصميم الذي سيستخدمه البرنامج آليا لطباعة هذا النمط
                    من الفواتير والسندات مع القدرة على اختيار تصميم آخر في أي وقت
                    أو معاينة الشكل المطبوع للفاتورة والسند وتحديد الحقول التي تريد طباعتها أو المتابعة دون طباعة
                </p>
                <dl>
                    <dt> بيانات الشركة   </dt>
                    <dd>

                             هنا يتم إدخال بيانات الشركة من اسم الشركة ,العنوان ,الترخيص ,أسماء الأشخاص ومعلومات التواصل معهم       
                    </dd>
                </dl>
                <dl>
                    <dt> خيارات الإظهار    </dt>
                    <dd>
                         هنا يتم تحديد خيارات الإظهار المتعلقة بالطباعة, بحيث يتم وضع إشارة على الخيار الذي يريده المستخدم أن يظهر 

                    </dd>
                </dl>
                <dl>
                    <dt> خيارات الطباعة    </dt>
                    <dd>
                     هنا يتم تحديد قياس الورق التي يريد المستخدم أن يتعامل بها مع السندات والفواتير والتقارير
                    </dd>
                </dl>

                <B class="c"> الأزرار :</B>
                <table>
                    <tr>
                        <th style="width:10%">
                                حفظ 
                        </th>
                        <td style="height:30px">
                            هذا الزر مخصص لحفظ الإعدادات المتعلقة بكامل البرنامج
                        </td>
                    </tr>
                

            </div>
        </section>

        <!-- </main> -->
    </div>

</body>

</html>

<?php
include('include/footer.php');
?>
<script>
    $('#content').onload(function(){
    $('#file_section').show();
    $('#file_sidebar').prop('class', 'active');

    $('#account_section').hide();
    $('#account_sidebar').removeClass('active');
    $('#item_section').hide();
    $('#item_sidebar').removeClass('active');
    $('#category_section').hide();
    $('#category_sidebar').removeClass('active');
    $('#bills_section').hide();
    $('#bills_sidebar').removeClass('active');
    $('#bonds_section').hide();
    $('#bonds_sidebar').removeClass('active');
    $('#setting_section').hide();
    $('#setting_sidebar').removeClass('active');

});

</script>

<script>
$('#file_sidebar').click(function() {
    $('#file_section').show();
    $('#file_sidebar').prop('class', 'active');

    $('#account_section').hide();
    $('#account_sidebar').removeClass('active');
    $('#item_section').hide();
    $('#item_sidebar').removeClass('active');
    $('#category_section').hide();
    $('#category_sidebar').removeClass('active');
    $('#bills_section').hide();
    $('#bills_sidebar').removeClass('active');
    $('#bonds_section').hide();
    $('#bonds_sidebar').removeClass('active');
    $('#setting_section').hide();
    $('#setting_sidebar').removeClass('active');

});

// $(document).ready(function() {
//     $('#file_section').show();
//     $('#file_sidebar').prop('class', 'active');

//     $('#account_section').hide();
//     $('#account_sidebar').removeClass('active');
//     $('#item_section').hide();
//     $('#item_sidebar').removeClass('active');
//     $('#category_section').hide();
//     $('#category_sidebar').removeClass('active');
//     $('#bills_section').hide();
//     $('#bills_sidebar').removeClass('active');
//     $('#bonds_section').hide();
//     $('#bonds_sidebar').removeClass('active');
//     $('#setting_section').hide();
//     $('#setting_sidebar').removeClass('active');

// });

$('#account_sidebar').click(function() {
    $('#account_section').show();
    $('#account_sidebar').prop('class', 'active');

    $('#file_section').hide();
    $('#file_sidebar').removeClass('active');
    $('#item_section').hide();
    $('#item_sidebar').removeClass('active');
    $('#category_section').hide();
    $('#category_sidebar').removeClass('active');
    $('#bills_section').hide();
    $('#bills_sidebar').removeClass('active');
    $('#bonds_section').hide();
    $('#bonds_sidebar').removeClass('active');
    $('#setting_section').hide();
    $('#setting_sidebar').removeClass('active');

});

$('#item_sidebar').click(function() {
    $('#item_section').show();
    $('#item_sidebar').prop('class', 'active');

    $('#file_section').hide();
    $('#file_sidebar').removeClass('active');
    $('#account_section').hide();
    $('#account_sidebar').removeClass('active');
    $('#category_section').hide();
    $('#category_sidebar').removeClass('active');
    $('#bills_section').hide();
    $('#bills_sidebar').removeClass('active');
    $('#bonds_section').hide();
    $('#bonds_sidebar').removeClass('active');
    $('#setting_section').hide();
    $('#setting_sidebar').removeClass('active');
});

$('#category_sidebar').click(function() {
    $('#category_section').show();
    $('#category_sidebar').prop('class', 'active');

    $('#file_section').hide();
    $('#file_sidebar').removeClass('active');
    $('#account_section').hide();
    $('#account_sidebar').removeClass('active');
    $('#item_section').hide();
    $('#item_sidebar').removeClass('active');
    $('#bills_section').hide();
    $('#bills_sidebar').removeClass('active');
    $('#bonds_section').hide();
    $('#bonds_sidebar').removeClass('active');
    $('#setting_section').hide();
    $('#setting_sidebar').removeClass('active');
});

$('#bills_sidebar').click(function() {
    $('#bills_section').show();
    $('#bills_sidebar').prop('class', 'active');

    $('#file_section').hide();
    $('#file_sidebar').removeClass('active');
    $('#account_section').hide();
    $('#account_sidebar').removeClass('active');
    $('#item_section').hide();
    $('#item_sidebar').removeClass('active');
    $('#category_section').hide();
    $('#category_sidebar').removeClass('active');
    $('#bonds_section').hide();
    $('#bonds_sidebar').removeClass('active');
    $('#setting_section').hide();
    $('#setting_sidebar').removeClass('active');
});

$('#bonds_sidebar').click(function() {
    $('#bonds_section').show();
    $('#bonds_sidebar').prop('class', 'active');


    $('#file_section').hide();
    $('#file_sidebar').removeClass('active');
    $('#account_section').hide();
    $('#account_sidebar').removeClass('active');
    $('#item_section').hide();
    $('#item_sidebar').removeClass('active');
    $('#bills_section').hide();
    $('#bills_sidebar').removeClass('active');
    $('#category_section').hide();
    $('#category_sidebar').removeClass('active');
    $('#setting_section').hide();
    $('#setting_sidebar').removeClass('active');
});

$('#setting_sidebar').click(function() {
    $('#setting_section').show();
    $('#setting_sidebar').prop('class', 'active');

    $('#file_section').hide();
    $('#file_sidebar').removeClass('active');
    $('#account_section').hide();
    $('#account_sidebar').removeClass('active');
    $('#item_section').hide();
    $('#item_sidebar').removeClass('active');
    $('#category_section').hide();
    $('#category_sidebar').removeClass('active');
    $('#bills_section').hide();
    $('#bills_sidebar').removeClass('active');
    $('#bonds_section').hide();
    $('#bonds_sidebar').removeClass('active');

});
</script>

<script>$('#file_sidebar').click(); </script>

<?php
    if(isset($_GET['file_section'])){
        echo "<script>$('#file_sidebar').click(); </script>";
    }
?>
<?php
    if(isset($_GET['account_section'])){
        echo "<script>$('#account_sidebar').click(); </script>";
    }
?>
<?php
    if(isset($_GET['item_section'])){
        echo "<script>$('#item_sidebar').click(); </script>";
    }
?>
<?php
    if(isset($_GET['category_section'])){
        echo "<script>$('#category_sidebar').click(); </script>";
    }
?>
<?php
    if(isset($_GET['bills_section'])){
        echo "<script>$('#bills_sidebar').click(); </script>";
    }
?>
<?php
    if(isset($_GET['bonds_section'])){
        echo "<script>$('#bonds_sidebar').click(); </script>";
    }
?>
<?php
    if(isset($_GET['setting_section'])){
        echo "<script>$('#setting_sidebar').click(); </script>";
    }
?>


<script>
    f1('help_file.php');
</script>